...
class AlertMonitor:
    ...
    def _load_state(self) -> Dict:
        if STATE_FILE.exists():
            try:
                ...
                logging.warning(f"⚠️ Помилка читання файлу стану: {e}. Ініціалізація нового стану.")
    ...
    def _save_state(self):
        try:
            ...
            logging.error(f"❌ Помилка збереження файлу стану: {e}")
    ...
    async def _spon_request(self, endpoint: str, payload: Dict) -> Optional[Dict]:
        try:
            ...
            logging.error(f"❌ Помилка запиту до {endpoint}: {e}")
        ...
        try:
            ...
            async with session.get(url, headers=self.alert_headers) as response:
                if response.status == 401:
                    logging.error("❌ Помилка авторизації (401): перевірте токен ALERT_TOKEN у .env файлі")
                    # print("❌ Помилка авторизації (401): перевірте токен ALERT_TOKEN у .env файлі")
                    return None
                elif response.status == 429:
                    logging.error("⚠️ Перевищено ліміт запитів (429): зачекайте")
                    # print("⚠️ Перевищено ліміт запитів (429): зачекайте")
                    return None
                elif response.status != 200:
                    logging.error(f"❌ HTTP помилка {response.status} при перевірці статусу тривог")
                    # print(f"❌ HTTP помилка {response.status} при перевірці статусу тривог")
                    return None
                response.raise_for_status()
                result = await response.json()
                if not isinstance(result, dict):
                    logging.warning(f"❌ Очікувався об'єкт, отримано {type(result)}")
                    # print(f"❌ Очікувався об'єкт, отримано {type(result)}")
                    return None
                last_action_index = result.get("lastActionIndex")
                if last_action_index is None:
                    logging.warning("⚠️ Відсутнє поле lastActionIndex у відповіді API")
            ...
            logging.error(f"❌ Помилка мережі при перевірці статусу тривог: {e}")
        ...
        if result and int(result.get("res",0)) == 1 and int(result.get("total",0)) > 0:
            for row in result.get("rows", []):
                if row.get("name") == filename:
                    ...
                    return True
        logging.error(f"❌ Файл '{filepath}' не знайдено на сервері SPON.")
        return False

    async def ensure_file_on_server(self, filename: str) -> bool:
        if await self.check_file_exists_on_server(filename):
            logging.info(f"✓ Файл '{filename}' вже є на сервері.")
            # print(f"✓ Файл '{filename}' вже є на сервері.")
            return True
        logging.error(f"❌ Файл '{filename}' не знайдено на сервері. Завантаження файлів виконується вручну.")
    ...
    async def get_file_duration(self, filepath: str) -> str:
        ...
        logging.warning("⚠️ Не вдалося отримати тривалість файлу, повертається 00:00:00")
        return "00:00:00"
    ...
    async def get_online_terminals(self) -> list:
        ...
        logging.warning("⚠️ Не вдалося отримати список онлайн терміналів")
        return []

    async def run_task(self, audio_file: str, region_name: str = "") -> Optional[str]:
        if not await self.ensure_file_on_server(audio_file):
            logging.error(f"❌ Неможливо створити задачу - файл {audio_file} недоступний.")
        ...
        if not (create_result and create_result.get("res") == '1'):
            logging.error(f"❌ Не вдалося створити задачу: {create_result}")
            # print(f"❌ Не вдалося створити задачу: {create_result}")
            return None

        task_id = create_result.get("taskId")
        if not task_id:
            logging.error(f"❌ Сервер не повернув taskId: {create_result}")
        ...
        if run_result and (run_result.get("res") == '1' or run_result.get("res") == 1):
            logging.info(f"✅ Задача [{region_name}] створена, запущена (ID: {task_id[:8]}...)")
            ...
            logging.info(f"⏳ Очікуємо завершення задачі {duration_sec + 1} секунд")
            # print(f"⏳ Очікуємо завершення задачі {duration_sec + 1} секунд")
            await asyncio.sleep(duration_sec + 1)
        else:
            logging.warning(f"⚠️ Задача створена (ID: {task_id[:8]}...), але не запущена: {run_result}")
        ...
        if del_result and del_result.get("res") == 1:
            logging.info(f"🗑️ Задача [{region_name}] видалена (ID: {task_id[:8]}...)")
            # print(f"🗑️ Задача [{region_name}] видалена (ID: {task_id[:8]}...)")
        else:
            logging.warning(f"⚠️ Не вдалося видалити задачу: {del_result}")
    ...
    async def check_region_status(self, region_id: int, retry_count: int = 0) -> Optional[Dict]:
        ...
        try:
            ...
            async with session.get(url, headers=self.alert_headers) as response:
                if response.status == 401 or response.status == 429:
                    if retry_count < max_retries:
                        ...
                        logging.error(f"❌ Регіон {region_id}: вичерпано кількість спроб через помилку {response.status}")
                        # print(f"❌ Регіон {region_id}: вичерпано кількість спроб")
                        return None
                elif response.status == 404:
                    logging.error(f"⚠️ Регіон {region_id} не знайдено (404)")
                    # print(f"⚠️ Регіон {region_id} не знайдено (404)")
                    return None
                elif response.status != 200:
                    logging.error(f"❌ HTTP помилка {response.status} для регіону {region_id}")
                ...
                if not isinstance(data, list) or len(data) == 0:
                    logging.error(f"❌ Неочікуваний формат даних для регіону {region_id}")
                ...
                logging.info(f"{status_icon} {region_data.get('regionName', 'Невідомо')}: {status_text}")
            ...
            logging.error(f"❌ Помилка при отриманні статусу регіону {region_id}: {e}")
            # print(f"❌ Помилка при отриманні статусу регіону {region_id}: {e}")
            return None

    async def check_all_regions(self) -> bool:
        logging.info(f"🔍 Перевірка статусу регіонів ({datetime.now():%H:%M:%S}):")
        ...
        for i, region_id in enumerate(REGION_IDS):
            ...
            if self.state["first_run"] is False and old_status != result["has_alert"]:
                ...
                logging.info(f"🔔 ЗМІНА СТАТУСУ: Region {region_id} {result['region_name']}")
                # print(f"\n🔔 ЗМІНА СТАТУСУ: Region {region_id} {result['region_name']}")
                logging.info(f"   Статус: {'ТРИВОГА' if result['has_alert'] else 'ВІДБІЙ'}")
                logging.info(f"   Оновлено: {result['last_update']}")
                ...
                logging.info(f"🔔 Перший запуск: оновлення статусу регіону {region_id} {result['region_name']}")
        ...
        if previous_main != any_alert:
            logging.info(f"🔄 Головний тригер оновлено: {any_alert}")
            # print(f"\n🔄 Головний тригер оновлено: {any_alert}")

        return changes_detected

    async def monitor_loop(self):
        logging.info("🚀 Запуск моніторингу повітряних тривог...")
        logging.info(f"📍 Відстежувані регіони: {REGION_IDS}")
        logging.info("ℹ️ Запити до API виконуються послідовно з затримкою для уникнення rate limiting")
        ...
        logging.info("✅ Перевірка файлів завершена")
        ...
        if self.state["first_run"]:
            ...
            if self.state.get("main_trigger"):
                logging.info("🚨 Перший запуск: головний тригер активний, запускаємо тривогу")
                # print("🚨 Перший запуск: головний тригер активний, запускаємо тривогу")
                await self.run_task(FILE_ON, "Глобальна тривога")
            else:
                logging.info("✅ Перший запуск: головний тригер неактивний, запускаємо відбій")
        ...
        try:
            while True:
                ...
                if current_action_index != self.state.get("last_action_index", 0):
                    logging.info(f"🔔 Виявлено зміну статусу тривог (index: {self.state.get('last_action_index', 0)} → {current_action_index})")
                    # print(f"\n[{datetime.now():%Y.%m.%d - %H:%M:%S}] 🔔 Виявлено зміну статусу тривог (index: {self.state.get('last_action_index', 0)} → {current_action_index})")
                    changes = await self.check_all_regions()
                    if changes:
                        logging.info(f"✅ Зміни оброблено у {STATE_FILE}")
                    ...
                    if new_main_trigger != last_main_trigger:
                        if new_main_trigger:
                            logging.info("🚨 Головний тригер активувався — запускаємо тривогу")
                            # print("🚨 Головний тригер активувався — запускаємо тривогу")
                            await self.run_task(FILE_ON, "Глобальна тривога")
                        else:
                            logging.info("✅ Головний тригер деактивувався — запускаємо відбій")
            ...
            logging.info("⛔ Моніторинг скасовано")
            # print("\n⛔ Моніторинг скасовано")
        except KeyboardInterrupt:
            logging.info("⛔ Зупинка моніторингу користувачем")
            # print("\n\n⛔ Зупинка моніторингу...")
        finally:
            if self.session and not self.session.closed:
                await self.session.close()
            logging.info("👋 Моніторинг завершено")
